...
def generate_scene_only(prompt, size="1024x1024"):
    """生成纯场景（没有人物）"""
...
def merge_character_scene(character_img, scene_img, prompt, size="1024x1024"):
    """将人物与场景融合"""
    url = f"{BASE_URL}/images/generations"
    headers = {
        "Content-Type": "application/json",
...
def test_single_shot():
    """测试单个镜头的两步生成"""
    # 测试场景
    scene_prompt = """中国动画电影风格，天庭斩仙台全景，黑色石质平台悬浮在云海中
十八根盘龙玉柱环绕，金色符文在平台边缘闪烁
天空阴沉云层厚重，远处是天宫建筑群
电影级光照，手绘质感，宏大场景，没有人"""
    ...
    if scene_url:
        # 保存场景图
        scene_path = "Scene-Platform-Test.png"
        save_image_from_url(scene_url, scene_path)

        # 融合人物
        merge_prompt = """将图1的人物（林玄）放入图2的场景中，跪在斩仙台中央
保持人物面容特征不变，融入场景氛围
中国动画电影风格，光影匹配"""
        ...
        if final_url:
            save_image_from_url(final_url, "Episode-01-Shot-001-TwoStep-Fixed.png")
